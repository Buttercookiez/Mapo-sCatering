...
// --- 2. TOAST NOTIFICATION (Updated to match Transactions.jsx + Portal) ---
const ToastNotification = ({ show, message, type, onClose }) => {
  ...
  // createPortal forces this div to render at the <body> level,
  // breaking it out of the overflow-hidden containers of the tabs/sidebar.
  return createPortal(
    <div
      className={`fixed bottom-8 right-8 z-[9999] flex items-center gap-3 px-6 py-4 rounded-sm shadow-xl border animate-in slide-in-from-right duration-300 font-sans
      ${
        isSuccess
...
const ConfirmModal = ({
  ...
  // We can also use Portal for the modal to ensure it's always on top
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all duration-300 font-sans">
      <div
...
const EventInfoTab = ({
  ...
  // --- 3. STATE FOR UI FEEDBACK (Matches Transactions.jsx structure) ---
  ...
  const isReservationPaid =
    secureStatuses.includes(currentStatus) || billing.paymentStatus === "Paid";
    ...
    openConfirm(
      ...
      async () => {
        ...
        try {
          ...
          showToast("50% payment recorded successfully.", "success");
  ...
  const handleSaveCost = async () => {
    ...
    try {
      ...
      showToast("Operational cost analysis saved.", "success");
      setTimeout(() => window.location.reload(), 1000);
    } catch (error) {
      showToast("Failed to save cost data.", "error");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto relative">
      {/* CONFIRM MODAL (Now uses Portal internally to appear on top) */}
      <ConfirmModal
      ...
      {isBookingRejected ? (
        // --- REJECTION VIEW ---
        <div
          className={`relative p-8 border ${theme.border} ${theme.cardBg} rounded-sm shadow-sm overflow-hidden transition-colors duration-500`}
        >
          ...
          <div className="flex items-start gap-5 mb-8">
            ...
            <div className="flex-1">
              ...
              <h3 className={`text-2xl font-serif italic ${theme.text} mt-1`}>
                Inquiry Rejected
              </h3>
              <p className={`text-sm ${theme.subText} mt-2 max-w-2xl`}>
                This inquiry has been marked for rejection.
              </p>
            </div>
          </div>
          {rejectionSent ? (
            <div
              className={`flex flex-col items-center justify-center py-12 border-t border-dashed ${theme.border}`}
            >
              ...
              {rejectionReason && (
                <div
                  className={`mt-6 p-4 bg-stone-50 dark:bg-stone-900/50 rounded-sm border ${theme.border} max-w-md w-full text-center`}
                >
            ...
            <div className="pl-0 md:pl-14">
              <textarea
                ...
                className={`w-full p-4 border ${theme.border} bg-transparent ${theme.text} placeholder-stone-500/50 rounded-sm focus:outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/20 transition-all duration-300 text-sm resize-none`}
                rows={5}
                placeholder="Reason..."
              />
              <div className="flex justify-end gap-4 mt-8">
                <button
                  onClick={() => handleUpdateStatus("Pending")}
                  className={`text-xs uppercase font-bold tracking-widest ${theme.subText}`}
                >
                  Cancel
                </button>
                <button
                  ...
                  className={`flex items-center gap-2 px-8 py-3 bg-red-600 text-white text-xs uppercase tracking-widest font-bold rounded-sm hover:bg-red-700 transition-all shadow-lg shadow-red-900/20 disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  {isSending ? (
        ...
        <>
          ...
          {isCancelled && (
            <div className="mb-8 p-6 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-sm flex flex-col md:flex-row gap-4 items-start relative overflow-hidden">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-serif font-bold text-red-700 dark:text-red-400">
                  Booking Cancelled
                </h4>
                <p className="text-sm text-red-600/80 dark:text-red-300 mt-1 leading-relaxed">
                  {details.cancellationReason || "Payment not settled."}
                </p>
          ...
          {/* --- FINANCIALS BLOCK --- */}
          <div
            className={`mt-8 mb-8 border ${theme.border} ${theme.cardBg} rounded-sm p-6 shadow-sm`}
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start mb-8 border-b border-dashed border-stone-200 dark:border-stone-800 pb-8">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span
                  className={`font-serif text-xl font-medium ${theme.text}`}
                >
              ...
              <div className="flex flex-col gap-1 md:border-l md:border-dashed md:border-stone-200 md:dark:border-stone-800 md:pl-8">
                ...
                <span
                  className={`font-serif text-2xl font-medium ${theme.text}`}
                >
                    ...
                    <div className="bg-stone-50 dark:bg-stone-900 p-3 rounded border border-stone-200 dark:border-stone-800">
                      ...
                      {is50PercentPaid && isActiveBooking && (
                        <div className="mt-2 pt-2 border-t border-dashed border-stone-200 dark:border-stone-700 flex items-center justify-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-wide">
                            50% Paid
                          </span>
                        </div>
                      )}
                        ...
                        <div className="flex gap-2">
                          {!is50PercentPaid && (
                            <button
                              ...
                              className="flex-1 flex items-center justify-center gap-2 bg-stone-700 hover:bg-stone-600 dark:bg-stone-800 dark:hover:bg-stone-700 text-white px-3 py-2 rounded-sm text-[10px] uppercase tracking-widest transition-colors shadow-sm"
                            >
                              {isUpdatingPayment ? (
                          ...
                          <button
                            ...
                            disabled={isUpdatingPayment}
                            className="flex-1 flex items-center justify-center gap-2 bg-[#1c1c1c] dark:bg-[#C9A25D] text-white dark:text-[#1c1c1c] hover:bg-emerald-600 dark:hover:bg-white px-3 py-2 rounded-sm text-[10px] uppercase tracking-widest transition-colors shadow-md font-bold"
                          >
                            {isUpdatingPayment ? (
                              <Loader2 size={12} className="animate-spin" />
                            ) : (
                      ...
                      <div className="flex flex-col gap-1 w-full">
                        {isCancelledOrRejected ? (
                          <div className="flex items-center justify-center gap-2 py-2 border border-red-200 bg-red-50 rounded-sm text-red-500 text-[10px] font-bold uppercase tracking-wide">
                            <XCircle size={12} /> {currentStatus}
                          </div>
                        ) : (
                          <p className="text-[10px] text-stone-400 italic flex items-center gap-1">
                            <AlertCircle size={10} /> Awaiting Reservation Fee
                          </p>
                        )}
                      </div>
                    )}
            ...
            <div className="rounded-sm overflow-hidden border border-stone-200 dark:border-stone-800 shadow-sm transition-colors duration-500">
              ...
              <div className="bg-[#1c1c1c] dark:bg-[#000000] border-b border-stone-800 p-4 flex justify-between items-center">
                <h4 className="text-[11px] uppercase tracking-[0.25em] text-[#C9A25D] font-bold flex items-center gap-2">
                  <TrendingUp size={14} className="text-[#C9A25D]" /> Internal
              ...
              {/* Body */}
              <div
                className={`p-6 grid grid-cols-1 md:grid-cols-3 gap-8 items-end bg-stone-100 dark:bg-[#0f0f0f] relative transition-colors duration-500 ${
                  !isEventCompleted ? "cursor-not-allowed" : ""
                }`}
              >
                ...
                {!isEventCompleted && (
                  <div className="absolute inset-0 bg-stone-100/50 dark:bg-black/60 backdrop-blur-[2px] z-10 flex items-center justify-center">
                    <div className="bg-white dark:bg-[#1a1a1a] px-5 py-3 rounded shadow-xl border border-stone-200 dark:border-stone-800 flex items-center gap-2">
                      ...
                      <span className="text-xs text-stone-500 dark:text-stone-300 italic">
                        Complete event to unlock analysis
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-col gap-2 z-0">
                  <label className="text-[10px] uppercase font-bold text-stone-500 dark:text-stone-500">
                    Operational Cost
                  </label>
                  <div className="flex items-end gap-2">
                    <div className="relative flex-1 group">
                      <span className="absolute left-0 bottom-2 text-sm text-stone-400 group-focus-within:text-[#C9A25D] transition-colors">
                        ₱
                      </span>
                      <input
                        type="number"
                ...
                <div className="flex flex-col gap-1 z-0">
                  <p className="text-[10px] uppercase font-bold text-stone-500 dark:text-stone-500">
                    Net Profit
                  </p>
                  <div className="flex items-baseline gap-2 border-b border-dotted border-stone-300 dark:border-stone-800 pb-1">
                    {hasOpsData ? (
                      ...
                      <span className="font-serif text-3xl font-bold text-stone-300 dark:text-stone-700">
                        ---
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-1 z-0">
                  ...
                  <div className="h-[46px] flex items-center">
                    {marginPercent !== null ? (
                      ...
                      <span className="text-sm text-stone-400 italic font-light">
                        Pending Input...
                      </span>
                    )}
                  </div>
      ...
      {/* RENDER TOAST - Now uses Portal to display outside the container */}
      <ToastNotification
        show={toast.show}
