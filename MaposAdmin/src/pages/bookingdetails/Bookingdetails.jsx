  ...
  return (
    <div className={`flex-1 h-full flex flex-col ${theme.bg} overflow-hidden`}>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Skeleton className="w-48 h-5" /> {/* Client Name */}
            <Skeleton className="w-24 h-3" /> {/* Ref ID */}
          </div>
        </div>
        <div className="flex gap-3">
          <Skeleton className="w-24 h-8 rounded-full" /> {/* Status Badge */}
          <Skeleton className="w-8 h-8 rounded-full" /> {/* Menu Button */}
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        >
          <Skeleton className="w-32 h-3 mx-auto mb-6" /> {/* Header */}
          <div className="flex flex-col items-center">
            <Skeleton className="w-16 h-16 rounded-full mb-3" /> {/* Avatar */}
            <Skeleton className="w-40 h-6 mb-2" /> {/* Name */}
          ...
          <Skeleton className="w-full h-24 rounded-sm" /> {/* Notes Area */}
        </div>
        ...
        <div className={`flex-1 flex flex-col ${theme.bg}`}>
          ...
          <div className="p-8 lg:p-12 max-w-4xl mx-auto w-full space-y-6">
            <Skeleton className="w-64 h-8 mb-6" /> {/* Title */}
            <div
...
const BookingDetails = ({
  ...
  // 1. Fetch Data Effect
  useEffect(() => {
    // --- ADDED: Reset tab to "Event Info" whenever a new booking is loaded ---
    // This ensures users always start at the info/approval screen
  ...
  const details = {
    ...
    dietary:
      currentData.initialNotes || currentData.dietary || "No specific requests",
    ...
    history:
      paymentData.history && paymentData.history.length > 0
        ...
        : [],
    timeline:
      notesData.timeline && notesData.timeline.length > 0
        ? notesData.timeline
        : [
            {
              date: "N/A",
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div
              className={`p-4 rounded-sm border ${theme.border} bg-transparent`}
            >
            ...
            <FadeIn key={activeDetailTab}>
              ...
              {activeDetailTab === "Event Info" && (
                <div className="max-w-4xl mx-auto">
                  {isBookingRejected ? (
                      ...
                      {rejectionSent ? (
                        <div className="flex flex-col items-center py-8 text-center">
                          ...
                          <p className={`text-sm ${theme.subText} mt-2`}>
                            The client has been notified regarding the rejection
                            of this inquiry.
                          </p>
                        </div>
                      ) : (
                        <>
                          <p className={`text-sm ${theme.text} mb-6`}>
                            This inquiry has been rejected. Please provide a
                            reason below to notify the client via email.
                          </p>
                    ...
                    <>
                      <h3 className={`font-serif text-2xl mb-6 ${theme.text}`}>
                        Event Specifications
                      </h3>
                      <div
                        className={`grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 p-8 border ${theme.border} ${theme.cardBg} rounded-sm shadow-sm`}
                      >
                        <div>
                          <p className="text-[10px] uppercase tracking-widest text-stone-400 mb-1">
                            Event Date
              ...
              {activeDetailTab === "Payments" && (
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    {[
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <stat.icon
                            size={18}
                            className="text-stone-300 dark:text-stone-600 group-hover:text-[#C9A25D] transition-colors"
                            strokeWidth={1.5}
                          />
                        </div>
                  ...
                  <div
                    className={`border ${theme.border} ${theme.cardBg} rounded-sm`}
                  >
                    <div
                      className={`flex justify-between items-center p-6 border-b ${theme.border}`}
                    >
                      <h4 className={`font-serif text-xl ${theme.text}`}>
                        Payment Schedule
                      </h4>
                    ...
                    <div
                      className={`grid grid-cols-12 gap-4 px-6 py-3 border-b ${theme.border} text-[10px] uppercase tracking-widest text-stone-400`}
                    >
                    ...
                    >
                      {[
                        ...
                        <div
                          key={i}
                          className={`grid grid-cols-12 gap-4 items-center px-6 py-5 ${theme.hoverBg} transition-colors duration-300 group`}
                        >
                          <div className="col-span-4 flex items-center gap-4">
                            <div
                              className={`p-2 rounded-full border ${theme.border} text-stone-400 bg-white dark:bg-transparent group-hover:border-[#C9A25D] group-hover:text-[#C9A25D] transition-colors`}
                            >
                              <row.icon size={16} />
                            </div>
                            <span
                              className={`text-sm font-bold ${theme.text} group-hover:text-[#C9A25D] transition-colors`}
                            >
                              {row.label}
                            </span>
                          </div>
                          <div className="col-span-3 text-xs text-stone-500 dark:text-stone-400">
                            Due: {row.due}
                          </div>
                          <div className="col-span-3 text-right">
                            <span
                              className={`font-serif text-lg font-medium ${theme.text}`}
                            >
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {menuPackages.map((pkg) => {
                          const isSelected =
                            proposalTotal / details.guests === pkg.price;
                          return (
                            <button
                              key={pkg.id}
                              onClick={() =>
                                setProposalTotal(pkg.price * details.guests)
                              }
                            ...
                            >
                              <span
                                className={`font-serif text-lg block ${theme.text}`}
                              >
                                {pkg.name}
                              </span>
                  ...
                  <div className="flex-1 relative space-y-8 before:absolute before:left-[19px] before:top-2 before:bottom-0 before:w-[1px] before:bg-stone-200 dark:before:bg-stone-800">
                    {details.timeline.map((log, i) => (
                      <div key={i} className="flex gap-6 relative">
                        <div
                          className={`w-10 h-10 rounded-full border ${theme.border} ${theme.cardBg} flex items-center justify-center z-10 shadow-sm`}
                        >
                          {log.user === "Admin" ? (
                            ...
                            <MessageSquare
                              size={16}
                              className="text-[#C9A25D]"
                            />
                          )}
                        </div>
                        <div
                          className={`flex-1 p-4 border ${theme.border} ${theme.cardBg} rounded-sm`}
                        >
