...
import { 
  Calendar, CheckSquare, AlertCircle, DollarSign, 
  Loader2, PieChart, Clock, MoreHorizontal, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const handlePrevDate = () => {
    ...
    else if (timeRange === 'Yearly') newDate.setFullYear(newDate.getFullYear() - 1);
  ...
  const getDisplayDateLabel = () => {
    if (timeRange === 'Weekly') {
      ...
      return `${startOfWeek.getDate()} ${startOfWeek.toLocaleDateString('default',{month:'short'})} - ${endOfWeek.getDate()} ${endOfWeek.toLocaleDateString('default',{month:'short'})}`;
    }
    if (timeRange === 'Monthly') return currentDate.toLocaleDateString('default', { month: 'long', year: 'numeric' });
    if (timeRange === 'Yearly') return currentDate.getFullYear().toString();
  };

  // --- Financial Data Processing Engine (Matches Financials.jsx Logic) ---
  const financialChartData = useMemo(() => {
    ...
    if (timeRange === 'Weekly') {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            buckets.push({
                label: d.toLocaleDateString('default', { weekday: 'narrow' }), // M, T, W
                fullLabel: d.toLocaleDateString('default', { weekday: 'short', day: 'numeric' }),
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            buckets.push({
                label: `${i}`,
                fullLabel: d.toLocaleDateString('default', { month: 'short', day: 'numeric' }),
                dateObj: new Date(d.setHours(0,0,0,0)),
                revenue: 0,
                expense: 0,
                profit: 0
        ...
        for (let i = 0; i < 12; i++) {
            ...
            buckets.push({
                label: d.toLocaleDateString('default', { month: 'narrow' }), // J, F, M
    ...
    validBookings.forEach(booking => {
        const bDate = new Date(booking.dateOfEvent || booking.date || booking.createdAt);
        if (isNaN(bDate)) return;

        // Financial Values
        const revenue = Number(booking.billing?.totalCost || booking.totalPrice || booking.amount || 0);
        const expense = Number(booking.billing?.operationalCost || 0); // Strict, no guessing
        ...
        if (timeRange === 'Weekly' || timeRange === 'Monthly') {
            ...
            matchIndex = buckets.findIndex(b => b.dateObj && b.dateObj.getTime() === bTime);
        } else if (timeRange === 'Yearly') {
  ...
  const formatK = (amount) => amount >= 1000 ? `₱${(amount / 1000).toFixed(1)}k` : `₱${amount}`;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className={`lg:col-span-2 border ${theme.border} ${theme.cardBg} p-8 shadow-sm`}>
                 <div className="flex flex-col md:flex-row justify-between md:items-start mb-10 gap-4">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-4">
                        <button onClick={handlePrevDate} className={`p-1 rounded-full ${theme.subText} hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors`}><ChevronLeft size={16}/></button>
                        <span className={`text-xs font-bold uppercase tracking-wider ${theme.text} min-w-[140px] text-center`}>{getDisplayDateLabel()}</span>
                        <button onClick={handleNextDate} className={`p-1 rounded-full ${theme.subText} hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors`}><ChevronRight size={16}/></button>
                      </div>
                    </div>
                 ...
                 <div className="flex items-end justify-between h-64 w-full gap-2 relative">
                    ...
                    {financialChartData.stats.length === 0 || financialChartData.stats.every(s => s.revenue === 0) ? (
                        <div className="w-full h-full flex flex-col items-center justify-center text-stone-400 z-20">
                            <span className="text-xs italic">No financial data for this period.</span>
                        </div>
                    ) : (
                      financialChartData.stats.map((item, i) => {
                        const heightPercent = (item.revenue / financialChartData.max) * 100;
                        const expensePercent = item.revenue > 0 ? (item.expense / item.revenue) * 100 : 0;
                        
                        return (
                          <div key={i} className="flex-1 flex flex-col justify-end h-full group relative cursor-pointer z-10 px-0.5">
                             {/* Tooltip */}
                             <div className="absolute -top-20 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 bg-stone-900 text-white px-3 py-2 rounded-sm shadow-xl z-20 pointer-events-none w-max border border-stone-700">
                                <span className="text-[9px] uppercase tracking-wider text-[#C9A25D] font-bold block mb-1 border-b border-stone-700 pb-1">{item.fullLabel}</span>
                                <div className="text-xs grid grid-cols-2 gap-x-3 gap-y-1">
                                  <span className="text-stone-400">Rev:</span> <span className="text-right text-white">{formatK(item.revenue)}</span>
                                  <span className="text-stone-400">Exp:</span> <span className="text-right text-stone-400">{formatK(item.expense)}</span>
                                  <span className="text-stone-400">Net:</span> <span className="text-right text-emerald-400">{formatK(item.profit)}</span>
                                </div>
                             ...
                             {/* Bars */}
                             <div style={{ height: `${heightPercent || 2}%` }} className={`w-full ${item.revenue > 0 ? 'bg-[#C9A25D]' : 'bg-stone-100 dark:bg-stone-800'} opacity-90 group-hover:opacity-100 group-hover:scale-y-105 transition-all duration-300 relative rounded-t-sm shadow-sm origin-bottom`}>
                                {/* Inner Bar = Expenses */}
                                <div style={{ height: `${expensePercent}%` }} className="absolute bottom-0 left-0 w-full bg-stone-400/50 dark:bg-stone-900/60 transition-all duration-500 border-t border-stone-500/20"></div>
                             </div>
                             <span className={`text-[9px] text-center mt-3 uppercase tracking-wider ${theme.subText} group-hover:text-[#C9A25D] transition-colors truncate w-full block`}>
                                {item.label}
                             </span>
                          </div>
                        );
