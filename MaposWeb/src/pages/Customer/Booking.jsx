...
const DropdownInput = ({ label, value, options, onSelect, placeholder, icon: Icon, theme, darkMode }) => {
  ...
  return (
    <div className="group relative z-30" ref={dropdownRef}>
      ...
      <button type="button" onClick={() => setIsOpen(!isOpen)} className={`w-full text-left bg-transparent border-b ${theme.border} py-5 text-xl ${theme.text} font-light font-sans flex justify-between items-center focus:outline-none focus:border-[#C9A25D] transition-colors`}>
...
const Booking = () => {
  ...
  const handleNextClick = (targetIndex) => { scrollToSection(targetIndex); };
  ...
  // --- CALENDAR LOGIC WITH 14-DAY BUFFER ---
  ...
  const renderCalendarDays = () => {
    ...
    // 1. Calculate the Minimum Bookable Date (Today + 14 Days)
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Normalize today to midnight
    
    const minBookableDate = new Date(today);
    minBookableDate.setDate(today.getDate() + 14); // Add 14 days for verification/prep
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      check.setHours(0, 0, 0, 0); // Normalize checked date to midnight
      
      // Formatting for comparison with bookedDates string array
      ...
      // 2. Logic: Disable if date is BEFORE the minimum bookable date
      // Example: Today=9, Min=23. If check is 22, 22 < 23 is TRUE (Disabled).
      ...
      if (isDisabled) textColor = darkMode ? '#57534e' : '#d1d5db'; // Disabled (Gray)
      ...
      days.push(
        <button 
          ...
          className={`
            ...
            ${isDisabled ? "line-through cursor-not-allowed opacity-50" : "hover:bg-[#C9A25D]/20 hover:text-[#C9A25D]"}
          `}
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[100] flex items-center justify-center ${darkMode ? 'bg-[#0c0c0c]' : 'bg-[#FAFAFA]'}`}
        style={{ clipPath: appLoading ? 'ellipse(150% 150% at 50% 50%)' : 'ellipse(150% 100% at 50% -100%)', transition: 'clip-path 1.2s cubic-bezier(0.76, 0, 0.24, 1)' }}>
      ...
      <div ref={containerRef} onScroll={handleNativeScroll} className={`h-screen w-full font-sans antialiased transition-colors duration-500 overflow-y-scroll md:overflow-hidden ${theme.bg} ${theme.text} selection:bg-[#C9A25D] selection:text-white`}>
        <style>{`
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                              ...
                              <div className="group relative">
                                  ...
                                  <div className={`flex items-end border-b ${theme.border} focus-within:border-[#C9A25D] transition-colors`}>
                                    ...
                                    <input type="text" name="phoneSuffix" value={formData.phoneSuffix} onChange={handlePhoneChange} className={`w-full bg-transparent py-3 text-xl ${theme.text} font-light focus:outline-none font-sans`} placeholder="XXX XXX XXX" required />
                                  </div>
                              </div>
                          </div>
                      </div>
              ...
              <div className="max-w-screen-xl mx-auto px-6 w-full">
                  <FadeIn>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                          {venues.map((venue) => (
                              <div key={venue.id} onClick={() => handlePredefinedVenueSelect(venue)} className={`group relative cursor-pointer flex flex-col h-[350px] md:h-[450px] transition-all duration-500 ${selectedVenue?.id === venue.id ? 'grayscale-0 translate-y-[-10px]' : 'grayscale hover:grayscale-0 hover:translate-y-[-10px]'}`}>
          ...
          <section ref={addToRefs} className={`min-h-screen md:h-screen flex flex-col justify-center py-20 ${theme.bg} border-t ${theme.border}`}>
               <div className="max-w-screen-lg mx-auto px-6 w-full">
                  <FadeIn>
                          ...
                          <div className="space-y-10">
                              <div className="group relative z-50">
                               ...
                               <div className="grid grid-cols-2 gap-8 z-30 relative">
                                  <DropdownInput label="Start Time" value={formData.startTime} options={timeOptions} onSelect={(val) => setFormData(prev => ({...prev, startTime: val}))} placeholder="Start" icon={Clock} theme={theme} darkMode={darkMode} />
                                  <DropdownInput label="End Time" value={formData.endTime} options={timeOptions} onSelect={(val) => setFormData(prev => ({...prev, endTime: val}))} placeholder="End" icon={Clock} theme={theme} darkMode={darkMode} />
                              </div>
                          </div>
                          <div className="space-y-10">
                               <div>
                      ...
                      <div className="mt-20">
                          <button type="submit" className={`group relative w-full py-6 text-sm tracking-[0.3em] uppercase font-light overflow-hidden transition-all duration-300 shadow-2xl ${darkMode ? 'bg-white text-black' : 'bg-black text-white'} font-sans`}>
                              ...
                              <span className="relative z-10 group-hover:text-white transition-colors duration-500">Request A Quotation</span>
                          </button>
                      </div>
